...
export default async function Dashboard() {
  ...
  try {
      ...
      return (
        <div className="flex items-center justify-center min-h-screen bg-slate-950 text-white p-6">
          <div className="glass-card p-12 text-center max-w-md">
            <h1 className="text-3xl font-black mb-4 text-rose-500 tracking-tighter">Core Linkage Fault</h1>
            <p className="text-slate-400 mb-8 border-l-2 border-rose-500/30 pl-6 text-left italic">The AI Forge could not establish a secure connection to the database core. This usually happens during high neural activity or system cold-starts.</p>
    ...
    if (!user) {
       return (
        <div className="flex items-center justify-center min-h-screen bg-slate-950 text-white p-6">
          <div className="glass-card p-12 text-center max-w-md border-amber-500/30">
            <h1 className="text-3xl font-black mb-4 text-amber-500 tracking-tighter">Identity Not Found</h1>
            <p className="text-slate-400 mb-8 text-sm leading-relaxed">Your authentication token is valid, but we couldn't find your record in the neural logs. This can happen if your account was purged or migrated.</p>
            <Link href="/api/auth/signout" className="btn-primary py-5 rounded-2xl bg-amber-600 hover:bg-amber-500">
               <span className="text-[10px] uppercase font-black tracking-widest">Protocol Reset (Log Out)</span>
            </Link>
          </div>
        </div>
      )
    ...
    if (isRedirectError(error)) throw error
    console.error("DASHBOARD_CRITICAL_FAULT:", error)
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-950 text-white p-6">
        <div className="glass-card p-12 text-center max-w-md">
          <h1 className="text-3xl font-black mb-4">Neural Component Fault</h1>
          <p className="text-slate-400 mb-8">A critical error occurred while rendering the dashboard interface. This has been logged for architectural review.</p>
          <Link href="/" className="btn-primary py-4 px-8">System Recovery</Link>
        </div>
      </div>
    )
  }
